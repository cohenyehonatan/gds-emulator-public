...
describe('LengthPrefixFraming', async () => {
  ...
  it('splits concatenated frames and keeps a partial remainder', async () => {
    const f = new LengthPrefixFraming();
...
describe('CrlfLineFraming', async () => {
  it('splits CRLF- and LF-delimited lines and drops empties', async () => {
    const f = new CrlfLineFraming();
    const { messages } = f.deframe(Buffer.from('SI*4321\r\n01Y1\n\n-SMITH/JOHN\r\n'));
    expect(messages).toEqual(['SI*4321', '01Y1', '-SMITH/JOHN']);
  });

  it('holds an unterminated line as remainder', async () => {
    const f = new CrlfLineFraming();
