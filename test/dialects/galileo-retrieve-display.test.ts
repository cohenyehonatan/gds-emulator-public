...
describe('Galileo *<arg> parsing', () => {
  it('parses *<locator> as display with locator argument', () => {
    const r = parseGalileoEntry('*ABCDEF');
    expect(r.kind).toBe('display');
    if (r.kind === 'display') expect(r.argument).toBe('ABCDEF');
  });

  it('parses *R / *I redisplay verbs', () => {
  ...
  it('parses *- WILLIAMS/CHRIS MR with internal whitespace preserved', () => {
    const r = parseGalileoEntry('*- WILLIAMS/CHRIS MR');
    if (r.kind === 'display') expect(r.argument).toBe('- WILLIAMS/CHRIS MR');
  });
});

describe('Galileo dialect — retrieve and display through the host', () => {
  let host: GdsHost;
  let wa: WorkArea;
  let locator: string;
  ...
  beforeEach(() => {
    ...
    locator = host.process('E', wa);
  });

  it('*R with no PNR returns NO BOOKING FILE', () => {
    expect(host.process('*R', wa)).toBe('NO BOOKING FILE');
  });

  it('*<locator> brings the BF into the active slot and renders it', () => {
    const resp = host.process(`*${locator}`, wa);
  ...
  it('*<unknown-locator> returns NO BOOKING FILE', () => {
    expect(host.process('*ZZZZZZ', wa)).toBe('NO BOOKING FILE');
    expect(wa.state()).toBe(SessionState.EMPTY);
  });

  it('*R after retrieve redisplays the same BF', () => {
    host.process(`*${locator}`, wa);
    const resp = host.process('*R', wa);
    expect(resp).toContain(locator);
    expect(resp).toContain('SMITH/JOHN MR');
  });

  it('*I after retrieve returns the itinerary only (no name lines, no phone)', () => {
    host.process(`*${locator}`, wa);
    const resp = host.process('*I', wa);
  ...
  it('*-<surname> with a unique match retrieves and renders the BF', () => {
    const resp = host.process('*-SMITH', wa);
    expect(resp).toContain(locator);
    expect(resp).toContain('SMITH/JOHN MR');
    expect(wa.pnr.locator).toBe(locator);
  });

  it('*-<unknown> returns NO BOOKING FILE', () => {
    expect(host.process('*-NOBODY', wa)).toBe('NO BOOKING FILE');
  });

  it('*-<surname> with multiple matches lists locators', () => {
    // Commit a second SMITH PNR
    host.process('IG', wa);
    host.process('A15JUNJFKLAX', wa);
    host.process('N1B1', wa); // book B class instead so seat isn't exhausted
    host.process('N.SMITH/JANE MS', wa);
    host.process('P.LON*02087654321', wa);
    host.process('T.TAU/10JUN', wa);
    host.process('R.AGT', wa);
    const locator2 = host.process('E', wa);
  ...
  it('ER (end + retrieve) renders the committed BF after building a new one', () => {
    // Start fresh: ignore, then build a new BF
    host.process('IG', wa);
    host.process('A15JUNJFKLAX', wa);
    host.process('N1B1', wa);
    host.process('N.DOE/JANE MS', wa);
    host.process('P.LON*02098765432', wa);
    host.process('T.TAU/10JUN', wa);
    host.process('R.AGT', wa);
    const resp = host.process('ER', wa);
    expect(resp).not.toMatch(/^[A-Z0-9]{6}$/); // not just a locator anymore
    expect(resp).toContain('DOE/JANE MS');
    expect(resp).toContain('JFK');
  });

  it('cross-area: retrieving in area B leaves A untouched', () => {
    host.process(`*${locator}`, wa);
    expect(wa.pnr.locator).toBe(locator);
    host.process('SB', wa);
