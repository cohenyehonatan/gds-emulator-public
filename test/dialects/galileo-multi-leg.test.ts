...
describe('Galileo multi-leg sell + connection-availability parsing', () => {
  it('parses N1Y1 as single-leg (legs absent)', () => {
  ...
  it('parses N2F1F2Y3 (multi-leg sell, 3 segments) populating legs', () => {
  ...
  it('parses A22JUNDXBSYD.SIN as connection availability via SIN', () => {
  ...
  it('parses A22JUNDXBSYD.SIN/SQ (via SIN, carrier SQ)', () => {
...
describe('Galileo multi-leg + connection through the host', () => {
  let host: GdsHost;
  let wa: WorkArea;
  ...
  it('A15JUNJFKSFO returns the seeded JFK→SFO connections (via ORD and DEN)', () => {
    const resp = host.process('A15JUNJFKSFO', wa);
    // Seeded inventory: JFK→ORD→SFO and JFK→DEN→SFO; both should appear
    expect(resp).toContain('ORD');
    expect(resp).toContain('DEN');
  });

  it('A15JUNJFKSFO.ORD restricts to connections via ORD only', () => {
    const resp = host.process('A15JUNJFKSFO.ORD', wa);
    expect(resp).toContain('ORD');
    expect(resp).not.toContain('DEN');
  });

  it('multi-leg N1Y<line1>Y<line2> sells both segments of a connection', () => {
    host.process('A15JUNJFKSFO.ORD', wa);
    // Lines 1 and 2 in the via-ORD list are JFK→ORD and ORD→SFO
    const resp = host.process('N1Y1Y2', wa);
  ...
  it('multi-leg sell where one class is unavailable rejects atomically (no partial state)', () => {
    host.process('A15JUNJFKSFO.ORD', wa);
    // Try a class that doesn't exist on line 1: 'Z'
    expect(host.process('N1Z1Y2', wa)).toBe('CLASS NOT AVAILABLE');
    expect(wa.pnr.segments.length).toBe(0); // no segments added
  });

  it('multi-leg sell where line number is invalid returns FORMAT', () => {
    host.process('A15JUNJFKSFO.ORD', wa);
    expect(host.process('N1Y1Y99', wa)).toBe('FORMAT');
    expect(wa.pnr.segments.length).toBe(0);
  });

  it('multi-leg sell decrements inventory for each leg', () => {
    host.process('A15JUNJFKSFO.ORD', wa);
    const before = host.process('A15JUNJFKSFO.ORD', wa); // redisplay shows seats
    host.process('N2Y1Y2', wa);
    const after = host.process('A15JUNJFKSFO.ORD', wa);
