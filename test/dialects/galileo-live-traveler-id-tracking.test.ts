...
describe('Galileo live N. — capture traveler UUID from addTraveler response', () => {
  ...
  it('N.SMITH/JOHN captures the traveler UUID into wa.liveTravelerIds', async () => {
    fetchSpy
      .mockResolvedValueOnce(tokenResponse())
      .mockResolvedValueOnce(searchResp())
      .mockResolvedValueOnce(createWb())
      .mockResolvedValueOnce(ok())                       // addOffer
      .mockResolvedValueOnce(travelerResp('uuid-john')); // addTraveler
  ...
  it('two N. entries capture two UUIDs in order', async () => {
    fetchSpy
      .mockResolvedValueOnce(tokenResponse())
      .mockResolvedValueOnce(searchResp())
      .mockResolvedValueOnce(createWb())
      .mockResolvedValueOnce(ok())
      .mockResolvedValueOnce(travelerResp('uuid-1'))
      .mockResolvedValueOnce(travelerResp('uuid-2'));
  ...
  it('a response without an Identifier still appends an empty slot to keep index alignment', async () => {
    fetchSpy
      .mockResolvedValueOnce(tokenResponse())
      .mockResolvedValueOnce(searchResp())
      .mockResolvedValueOnce(createWb())
