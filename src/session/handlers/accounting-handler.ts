...
import type { AccountingDeleteEntry } from '../../protocol/entry.js';
...
export function handleAccountingDelete(entry: AccountingDeleteEntry, wa: WorkArea): string {
  ...
  if (pnr.tickets.length === 0) return 'NO ACCOUNTING DATA'; // nothing to delete

  if (entry.mode === 'all') {
    for (let n = 1; n <= pnr.tickets.length; n++) pnr.accountingLinesHidden.add(n);
  ...
  for (const n of entry.lines) {
    if (n < 1 || n > pnr.tickets.length) return 'ACCOUNTING LINE NOT FOUND'; // reconstructed
  }
